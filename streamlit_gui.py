...
def feature_engineering(uploaded_data):
    ...
    # Calculate BMI
    modified_data['BMI'] = modified_data['Weight'] / (modified_data['Height'] ** 2)
    modified_data['BMI'] = round(modified_data['BMI']).astype(int)

    for dataset in [modified_data]:
        dataset.loc[(dataset['BMI'] <= 18), 'BMI'] = 0
        dataset.loc[(dataset['BMI'] > 18) & (dataset['BMI'] <= 25), 'BMI'] = 1
        dataset.loc[(dataset['BMI'] > 25) & (dataset['BMI'] <= 40), 'BMI'] = 2
        dataset.loc[(dataset['BMI'] > 40), 'BMI'] = 3

        dataset['BMI'] = dataset['BMI'].astype(str)
        dataset.loc[dataset['BMI'] == '0', 'BMI'] = "Underweight"
        dataset.loc[dataset['BMI'] == '1', 'BMI'] = "Normal"
        dataset.loc[dataset['BMI'] == '2', 'BMI'] = "Overweight"
        dataset.loc[dataset['BMI'] == '3', 'BMI'] = "Obese"
...
def categorize_obesity(modified_data):
    ...
    if 'BMI' in modified_data.columns:
        for dataset in [modified_data]:
            dataset.loc[(dataset['BMI'] == 'Underweight') , 'BMI'] = 0
            dataset.loc[(dataset['BMI'] == 'Normal'), 'BMI'] = 1
            dataset.loc[(dataset['BMI'] == 'Overweight'), 'BMI'] = 2
            dataset.loc[(dataset['BMI'] == 'Obese'), 'BMI'] = 3

            dataset['BMI'].astype(int)
            dataset.loc[ dataset['BMI'] == 0, 'BMI'] = 0
            dataset.loc[ dataset['BMI'] == 1, 'BMI'] = 0
            dataset.loc[ dataset['BMI'] == 2, 'BMI'] = 1
            dataset.loc[ dataset['BMI'] == 3, 'BMI'] = 1
...
def split_data(patientdf):
    if 'Obesity_Level' in patientdf.columns:
        y = patientdf["Obesity_Level"].values
        x = patientdf.drop(["Obesity_Level"],axis=1).values
    else:
        y = patientdf["BMI"].values
        x = patientdf.drop(["BMI"],axis=1).values
